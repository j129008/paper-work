    ...
    while True:
        ...
        for i in range(len(y)-seq_len):
            ...
            if len(batch_x) == batch_size:
                _batch_x = np.array(batch_x)
                _batch_y = np.array(batch_y)
                batch_x = []
                batch_y = []
                yield (_batch_x, _batch_y)

def predict_gen(x, seq_len, batch_size=100):
    while True:
        batch_x = []
...
for k in range(9, 15):
    ...
    model.fit_generator(batch_gen(x_train, y_train, seq_len=seg_size, batch_size=100), steps_per_epoch=len(y_train)//5, epochs=100, validation_data=batch_gen(x_vaild, y_vaild, seq_len=seg_size), validation_steps=len(y_vaild)//5, callbacks=[early_stop])
    pred = model.predict_generator(predict_gen(x_test, seq_len=seg_size), steps=(len(y_test)//100)+1, verbose=1)
    pred = list([ seq[0] for seq in pred ]) + list(pred[-1][1:])
    union = lambda data: [ ins for chap in data for ins in chap ]
    y_pred = data.y2lab(union(pred))
    y_test = data.y2lab(union(y_test))
    print(metrics.flat_classification_report(
        y_test, y_pred[:len(y_test)], labels=('I', 'E'), digits=4
